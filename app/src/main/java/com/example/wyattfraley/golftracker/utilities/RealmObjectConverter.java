...
public class RealmObjectConverter {
    ...
    public static final RealmRoundScore toRealmRoundScore(SerializableRoundScore roundData) {
        ...
        for (int i = 0; i < 18; i++) {
            RealmHoleScore holeScore = new RealmHoleScore(roundData.getStrokes().get(i), roundData.getPutts().get(i), roundData.getPenalties().get(i), roundData.getSand().get(i), roundData.getFairway().get(i), roundData.getGreenInRegulation().get(i), 0, 0);
